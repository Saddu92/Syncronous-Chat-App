...
import { GET_ALL_MESSAGES_ROUTE, GET_CHANNELS_MESSAGES, HOST } from "@/lib/utils/constants";
...
function MessageContainer() {
  ...
  const downloadFile = async (url) => {
    ...
    const res = await apiClient.get(`${HOST}/${url}`, {
      responseType: "blob", // blob matlb file ko binary file jaisa treat karo na ki  json
...
{message.messageType === "file" && (
  ...
  return (
    <div className="flex-1 overflow-y-auto scrollbar-hidden p-4 px-8 md:w-[65vw] lg:w-[70vw] xl:w-[80vw] w-full">
      ...
      <div ref={scrollRef} />
      {showImage && (
        <div className="fixed z-[1000] top-0 left-0 h-screen w-screen flex flex-col items-center justify-center backdrop-blur-lg p-6">
          <img
            src={`${HOST}/${imageUrl}`}
            alt="Preview"
            className="max-h-[80vh] w-[90vw] object-contain rounded-lg shadow-lg"
          />

          <div className="flex gap-5 mt-6">
            <button
              className="bg-black/40 text-white p-3 text-2xl rounded-full hover:bg-black/70 cursor-pointer transition-all duration-300"
              onClick={() => downloadFile(imageUrl)} // ✅ only run on click
            >
              <IoMdArrowRoundDown />
            </button>
